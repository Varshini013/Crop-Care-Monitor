    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            const [statsRes, historyRes, activityRes] = await Promise.all([
                // UPDATED: URLs are now relative for deployment
                axios.get('/api/predict/stats', config),
                axios.get('/api/predict/history', config),
                axios.get('/api/predict/activity', config)
            ]);
                        ...
                        <div className="space-y-3">
                            {history.length > 0 ? history.map(item => (
                                <div key={item._id} className="flex items-center text-sm">
                                    <img src={`/api/${item.imageUrl.replace('uploads/', '')}`} alt="" className="h-8 w-8 rounded-md object-cover mr-3"/>
