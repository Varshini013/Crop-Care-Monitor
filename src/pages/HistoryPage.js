            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl" onClick={e => e.stopPropagation()}>
                <div className="p-6 relative">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        {/* UPDATED: Image URL is now relative */}
                        <img src={`/api${item.imageUrl}`} alt={item.diseaseName} className="w-full h-auto rounded-lg object-cover border"/>
    ...
    return (
        <div className={`bg-white rounded-xl shadow-md border overflow-hidden transform hover:-translate-y-1 hover:shadow-xl transition-all duration-300 relative ${isSelected ? 'border-green-500 ring-2 ring-green-500' : 'border-gray-200'}`}>
            ...
            <div className="grid grid-cols-3">
                {/* UPDATED: Image URL is now relative */}
                <div className="col-span-1"><img src={`/api${item.imageUrl}`} alt={item.diseaseName} className="h-full w-full object-cover"/></div>
    ...
    useEffect(() => {
        const fetchHistory = async () => {
            ...
            try {
                ...
                // UPDATED: API call is now relative
                const { data } = await axios.get('/api/predict/history', config);
                setHistory(data);
            } catch (err) {
    ...
    const handleDeleteSelected = async () => {
        if (window.confirm(`Are you sure you want to delete ${selectedIds.size} item(s)?`)) {
            try {
                ...
                // UPDATED: API call is now relative
                await axios.delete('/api/predict', config);
